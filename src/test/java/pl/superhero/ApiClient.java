...
public class ApiClient {
    ...
    /**
     * Call listHeroes with empty body. 
    ...
    /**
     * Call createHero with empty body. 
     * @param handler The handler for the asynchronous request
     */
    ...
    /**
     * Call showHeroById with empty body. 
     * @param heroId Parameter heroId inside path
     * @param handler The handler for the asynchronous request
     */
    public void showHeroById(
        String heroId,
        Handler<AsyncResult<HttpResponse>> handler) {
        // Check required params
        if (heroId == null) throw new RuntimeException("Missing parameter heroId in path");
        

        // Generate the uri
        String uri = "/heroes/{heroId}";
        uri = uri.replaceAll("\\{{1}([.;?*+]*([^\\{\\}.;?*+]+)[^\\}]*)\\}{1}", "{$2}"); //Remove * . ; ? from url template
        uri = uri.replace("{heroId}", this.renderPathParam("heroId", heroId));
